...
class ReliableDataTransfer:    
    def __init__(self, retries=3):
        ...
        self.len_checksum = 14 # length of checksum
        self.re_checksum = r'^///[0-9a-fA-F]{8}\\\\\\$' # regular expression for checksum

    def checksum(self, chunk):
        # generate a checksum for the chunk of data
        return crc32(chunk) & 0xffffffff
    
    def process_data(self, data):
        print(data)
        # split data into chunk and checksum
        chunk, rcs = data[:-1*self.len_checksum], data[-1*self.len_checksum:].decode('utf-8')
        
        # return false if checksum is missing or if checksums don't match else true
        if re.match(self.re_checksum, rcs):
            rcs = rcs.strip("/\\")
            has_error = True if self.checksum(chunk) != int(rcs, 16) else False
        else:
            has_error = False
    ...
    def run_server(self, outdir, addr, mtu):
        ...
        while True:
            ...
            data,remote_addr = sock.recvfrom(mtu + self.len_checksum)
            ...
            if in_xfr:
                ...
                if data[:9] == B"///END\\\\\\":
                    ...
                    chunk, has_error = self.process_data(data)
                    # if no checksum or incorrect checksum
                    if has_error:
                        log.info("Incorrect or missing CHECKSUM.")
                        continue

                    # else we got a chunk of data
                    log.debug("Got a correct chunk!")
    ...
    def xfr(self, sock, payload, dest, mtu):
        ...
        for i,chunk in enumerate(payload):
            ...
            while tries < self.retries:
                # add checksum to the chunk of data
                cs = "///{:08x}\\\\\\".format(
                    self.checksum(chunk)
                ).encode('utf-8')
                # send the chunk
                sock.sendto(chunk + cs, dest)
                try:
                    # wait for an ACK
                    data, addr = sock.recvfrom(mtu)
                except socket.timeout:
    ...
    def send_file(self, filename, dest, mtu):
        "Entrypoint for sending"
        st = datetime.now()
        log.info("Sending with custom reliable-data-transfer {} --> {}:{} [MTU={}].".format(
        ...
        log.info("Sent with custom reliable-data-transfer {} in {} seconds = {:.0f} bps.".format(
            filename, seconds,
            total_bytes / seconds))
